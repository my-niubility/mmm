 ...
 * @description :产品发布审核
 */
@RequestMapping(value= "/pubcheck")
@Controller
public class ProductPubCheckController {

	private final static Logger logger = LoggerFactory.getLogger(ProductPubCheckController.class); 
	...
	public String  query(HttpServletRequest request, HttpServletResponse response){
		...
		cb.setProductStatus(ProductStatus.PRODUCT_STATUS01.getValue());//发布待审核
		//根据前端每页展示数据条数
		...
		return "/WEB-INF/view/product/product_pubcheck_body";
	 ...
	 * @description:默认进入处理
	 */
	@RequestMapping(value= "/product_pubcheck_body",method = {RequestMethod.POST, RequestMethod.GET})
	public String  queryBody(HttpServletRequest request, HttpServletResponse response){
		logger.info("/product_pubcheck_body===");
	...
	public String  queryProductDetail(HttpServletRequest request, HttpServletResponse response){
		...
		return "/WEB-INF/view/product/product_pubcheck_detail";
	...
	public String  checkResult(HttpServletRequest request, HttpServletResponse response){
		...
		reqDto.setType(ProductAuditType.RELEASE_AUDIT);
		reqDto.setPassed("1".equals(checkResult)?ProductAuditResult.PASSED:ProductAuditResult.NOTPASSED);
		boolean addFlag = productCheckService.productPublishAudit(reqDto);
		if(addFlag){
			//成功
	...
	public String  queryMain(HttpServletRequest request, HttpServletResponse response){
		...
		return "/WEB-INF/view/product/product_pubcheck_main";
		
	}
	
	@RequestMapping(value= "/product_pubcheck_top",method = {RequestMethod.POST, RequestMethod.GET})
	public String  queryTop(HttpServletRequest request, HttpServletResponse response){
		logger.info("/product_query_top===");
		return "/WEB-INF/view/product/product_pubcheck_top";
